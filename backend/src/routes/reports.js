...
router.get('/sales', authRequired, async (_req, res) => {
  const byProduct = await query(`
    SELECT p.id, p.name, COALESCE(SUM(t.quantity), 0)::int AS quantity_sold
    FROM products p
  ...
  const byDistributor = await query(`
    SELECT d.id, d.name, COALESCE(SUM(t.quantity), 0)::int AS quantity_sold
    FROM distributors d
  ...
  const byDistributorProduct = await query(`
    ...
    ORDER BY d.name, quantity_sold DESC
  `);
